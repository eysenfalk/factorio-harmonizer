...
-- Fixes: MISSING_DEPS_ELECTRONIC-CIRCUIT
...
-- Patch: PATCH_ADVANCED-CIRCUIT_ALL_VARIANTS
-- Description: Recipe expansion for advanced-circuit - adds all mod variants as additional recipes
-- Fixes: MISSING_DEPS_ADVANCED-CIRCUIT


-- Comprehensive recipe expansion for advanced-circuit
-- Adds mod-specific recipe variants alongside original recipes
-- Affected mods: base
-- Severity: HIGH
...
-- Base Game variant of advanced-circuit
if data.raw.recipe["advanced-circuit"] then
    local base_variant = table.deepcopy(data.raw.recipe["advanced-circuit"])
    base_variant.name = "advanced-circuit-base-variant"
    base_variant.localised_name = {"", "advanced-circuit", " (Base Game Style)"}
    base_variant.enabled = true
    base_variant.order = (base_variant.order or "a") .. "-base-variant"
    base_variant.hidden = false

    base_variant.ingredients = {{type="item", name="electronic-circuit", amount=2}, {type="item", name="plastic-bar", amount=2}, {type="item", name="copper-cable", amount=4}}
    base_variant.results = {{type="item", name="advanced-circuit", amount=1}}
...
-- Keep ALL original recipes active - no disabling!
-- Players can now choose between:
-- 1. Original advanced-circuit (current winner of mod conflicts)
-- 2. advanced-circuit-base-variant (Base Game style)
...
-- Fixes conflict between: base, wood-military
-- Severity: MEDIUM
-- Type: item
...
-- Fixes conflict between: wood-industry, base, wood-military
-- Severity: MEDIUM
-- Type: item
